...
HEADER_PATTERNS = {
    "num_quarters_used": ("num quarters", "num_q", "n quarters", "n qtr", "quarters used"),
...
def to_int(value: Any, fallback: int) -> int:
    ...
    return fallback if number is None else int(round(number))
...
def load_sheet_snapshot(sheet: xw.Sheet) -> SheetSnapshot:
    ...
    return SheetSnapshot(start_row=used.row, start_col=used.column, values=to_2d(used.value))
...
def find_max_anchor(snapshot: SheetSnapshot) -> Optional[Tuple[int, int]]:
    ...
    for row_idx, row in enumerate(snapshot.values):
        for col_idx, value in enumerate(row):
            ...
            right_text = normalize_text(snapshot_value(snapshot, abs_row, abs_col + 1))
            bonus = 0 if right_text == "min" else 1
        ...
        for col in range(col_start, col_end + 1):
            ...
            for key, patterns in HEADER_PATTERNS.items():
                ...
                if prev is None or distance < prev[0]:
                    found[key] = (distance, col)

    resolved = {key: value[1] for key, value in found.items()}
    resolved.setdefault("forecast_max", anchor_col)
    if "forecast_min" not in resolved:
        right_text = normalize_text(snapshot_value(snapshot, anchor_row, anchor_col + 1))
        resolved["forecast_min"] = anchor_col + 1 if right_text == "min" else anchor_col - 1
    return resolved
...
def collect_candidate_rows(
    ...
    for row in range(anchor_row + 1, min(anchor_row + limit, snapshot.end_row) + 1):
        nearby_values = [
            ...
            for col in range(
                max(snapshot.start_col, anchor_col - 2),
                min(snapshot.end_col, anchor_col + 2) + 1,
            )
        ]
        if any(value is not None and str(value).strip() != "" for value in nearby_values):
            rows.append(row)
    return rows[:limit]
...
def extract_empirical_rows(wb: xw.Book, model_fields: Dict[str, str], source_file: str) -> List[Dict[str, Any]]:
    ...
    avg_pen_map: Dict[int, Any] = {}
    if formula_rows:
        wb.app.calculate()
        values = sheet.range((formula_rows[0], helper_col), (formula_rows[-1], helper_col)).value
        for row, value in zip(formula_rows, to_1d(values, len(formula_rows))):
            avg_pen_map[row] = value
        sheet.range((formula_rows[0], helper_col), (formula_rows[-1], helper_col)).clear_contents()

    rows: List[Dict[str, Any]] = []
    for idx, row in enumerate(candidate_rows):
        num_quarters = to_int(snapshot_value(snapshot, row, cols["num_quarters_used"]), idx + 1)
        avg_pen = first_not_none(avg_pen_map.get(row), snapshot_value(snapshot, row, cols["avg_penetration_pct"]))
...
def find_xy_data_block(
    ...
    while row >= snapshot.start_row:
        if is_number(snapshot_value(snapshot, row, x_col)) and is_number(snapshot_value(snapshot, row, y_col)):
            break
        row -= 1
    ...
    while start_row - 1 >= snapshot.start_row:
        prev_x = snapshot_value(snapshot, start_row - 1, x_col)
        prev_y = snapshot_value(snapshot, start_row - 1, y_col)
        if not (is_number(prev_x) and is_number(prev_y)):
            break
        start_row -= 1
...
def extract_regression_rows(wb: xw.Book, model_fields: Dict[str, str], source_file: str) -> List[Dict[str, Any]]:
    ...
    intercept_vals = sheet.range((anchor_row + 1, intercept_col), (anchor_row + max_rows, intercept_col)).value
    slope_vals = sheet.range((anchor_row + 1, slope_col), (anchor_row + max_rows, slope_col)).value
    intercept_list = to_1d(intercept_vals, max_rows)
    slope_list = to_1d(slope_vals, max_rows)
...
def write_table(worksheet, columns: Sequence[str], rows: Iterable[Dict[str, Any]]) -> None:
