...
def set_formula2(cell: Any, formula: str) -> None:
    try:
        ...
        # Fallback for older Excel builds without Formula2 support.
...
def parse_month_token(month_token: str) -> Optional[int]:
...
def parse_file_label(file_name: str) -> dict[str, str]:
    ...
    match = re.search(
        r"(Early|Mid|Late)\s*([A-Za-z]{3,12})\s*(\d{4})",
        period_token,
        flags=re.IGNORECASE,
    )
    if match:
        ...
        if month_num is not None:
            month_abbr = date(year, month_num, 1).strftime("%b")
...
def process_regression_sheet(
    ...
    for idx, (row, fallback_num_quarters) in enumerate(rows_to_read):
        ...
        signature = compact_signature([intercept, slope, forecast_value, forecast_max, forecast_min])
