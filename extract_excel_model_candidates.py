...
def extract_empirical_rows(workbook: xw.Book, labels: FileLabels, source_file: str) -> List[Dict[str, Any]]:
    ...
    for idx in range(1, N_QUARTERS + 1):
        ...
        if columns["quarterly_sales"] and columns["reported_sales"]:
            fallback_avg_col = columns["avg_penetration_pct"] or columns["forecast_max"]
            formula = (
                f"=IFERROR(RC{columns['quarterly_sales']}/RC{columns['reported_sales']},"
                f"RC{fallback_avg_col})"
            )
    ...
    for idx in range(1, N_QUARTERS + 1):
        ...
        avg_penetration_pct = sheet.cells(row, temp_avg_col).value if row in formula_row_set else None
    ...
    if formula_rows:
        sheet.range((formula_rows[0], temp_avg_col), (formula_rows[-1], temp_avg_col)).clear_contents()
...
def extract_regression_rows(workbook: xw.Book, labels: FileLabels, source_file: str) -> List[Dict[str, Any]]:
    ...
    for idx in range(1, N_QUARTERS + 1):
        ...
        forecast_max = sheet.cells(row, columns["forecast_max"]).value if columns["forecast_max"] else None
        forecast_min = sheet.cells(row, columns["forecast_min"]).value if columns["forecast_min"] else None
        intercept = sheet.cells(row, temp_intercept_col).value if row in formula_row_set else None
        slope = sheet.cells(row, temp_slope_col).value if row in formula_row_set else None
...
def set_sheet_style(worksheet: Worksheet, headers: Sequence[str], max_width: int = 60) -> None:
    for col_idx in range(1, len(headers) + 1):
        worksheet.cell(row=1, column=col_idx).font = Font(bold=True)
    worksheet.freeze_panes = "A2"
    worksheet.auto_filter.ref = worksheet.dimensions

    for col_idx, width in enumerate(_compute_widths(worksheet, headers), start=1):
        worksheet.column_dimensions[get_column_letter(col_idx)].width = min(width + 2, max_width)


def _compute_widths(worksheet: Worksheet, headers: Sequence[str]) -> List[int]:
...
def write_sheet(worksheet: Worksheet, headers: Sequence[str], data_rows: Sequence[Dict[str, Any]]) -> None:
    ...
    set_sheet_style(worksheet, headers)
