...
def parse_file_labels(file_path: Path) -> FileLabels:
    ...
    if period_match:
        ...
        try:
            ...
            clean_period = re.sub(r"\s+", "_", period_source).strip("_")
            model_period = clean_period or "UnknownPeriod"
            model_date = ""
    else:
        clean_period = re.sub(r"\s+", "_", period_source).strip("_")
        model_period = clean_period or "UnknownPeriod"
...
def extract_empirical_candidates(
    ...
    helper_values = [
        safe_value(sheet.range((first_data_row + idx, helper_col)).value)
        for idx in range(n_quarters)
    ]
    sheet.range(
...
def extract_regression_candidates(
    ...
    for idx, _n in enumerate(n_values):
        row = first_data_row + idx
        n = n_values[idx]
