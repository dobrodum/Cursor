...
def parse_month_token(month_token: str) -> tuple[str, int] | None:
    cleaned = month_token.strip().lower()
    if not cleaned:
        return None
    month_number = MONTH_TO_NUMBER.get(cleaned)
    if month_number is None and len(cleaned) >= 3:
        month_number = MONTH_TO_NUMBER.get(cleaned[:3])
    if month_number is None:
        return None
    month_abbrev = calendar.month_abbr[month_number]
    return month_abbrev, month_number
    ...
    for row_index, row in enumerate(snapshot.values):
        for col_index, value in enumerate(row):
            if isinstance(value, str) and value.strip().lower() == target:
                return snapshot.start_row + row_index, snapshot.start_col + col_index
    return None
...
def extract_empirical_candidates(
    ...
    relative_source_col = penetration_source_col - helper_col

    for idx in range(n_quarters):
        row = first_data_row + idx
        formula = (
            f'=IFERROR(AVERAGE(R[{-idx}]C[{relative_source_col}]:RC[{relative_source_col}]), "")'
        )
    ...
    for idx in range(n_quarters):
        row = first_data_row + idx
        num_quarters_used = as_int(snapshot.get(row, anchor_col + offsets["num_quarters_used"])) or (idx + 1)
        ...
        avg_penetration = safe_value(snapshot.get(row, anchor_col + offsets["avg_penetration_pct"]))
        if avg_penetration is None:
            avg_penetration = helper_values[idx]
...
def extract_regression_candidates(
    ...
    for row in range(anchor_row - 1, snapshot.start_row - 1, -1):
        y_value = as_number(snapshot.get(row, y_col))
        x_value = as_number(snapshot.get(row, x_col))
...
def write_sheet(
    ...
    for col_idx in range(1, worksheet.max_column + 1):
        ...
        worksheet.column_dimensions[get_column_letter(col_idx)].width = min(max(max_len + 2, 12), 44)
...
def run() -> None:
    ...
    write_sheet(output_workbook, "empirical_candidates", EMPIRICAL_HEADERS, empirical_rows)
    write_sheet(output_workbook, "regression_candidates", REGRESSION_HEADERS, regression_rows)
