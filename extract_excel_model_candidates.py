...
def safe_close_workbook(wb: xw.Book) -> None:
    ...
    close_attempts: list[tuple[str, callable]] = [
        ("wb.close(save=False)", lambda: wb.close(save=False)),
        ("wb.close(save_changes=False)", lambda: wb.close(save_changes=False)),
        ("wb.api.Close(SaveChanges=False)", lambda: wb.api.Close(SaveChanges=False)),  # type: ignore[attr-defined]
        ("wb.api.Close(False)", lambda: wb.api.Close(False)),  # type: ignore[attr-defined]
    ]
    for _, closer in close_attempts:
        try:
            closer()
            return
        except Exception:
    ...
    print("Warning: workbook close fallback exhausted; source file may still be open")
...
def set_formula2_r1c1(cell: xw.Range, formula_r1c1: str) -> None:
    """Prefer Formula2 R1C1, with safe fallbacks."""
    try:
