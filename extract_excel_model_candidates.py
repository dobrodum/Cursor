...
def build_sheet_snapshot(sheet: xw.Sheet) -> SheetSnapshot:
    ...
    for row in values:
        padded_rows.append(list(row) + [None] * (max_cols - len(row)))
    return SheetSnapshot(
        start_row=used.row,
        start_col=used.column,
        end_row=used.row + len(padded_rows) - 1,
...
def parse_file_labels(file_path: Path) -> FileLabels:
    ...
    match = re.search(r"(Early|Mid|Late)([A-Za-z]{3,})(\d{4})", period_source, re.IGNORECASE)
    if match:
        period_part = match.group(1).title()
        month_abbrev = match.group(2)[:3].title()
        ...
        try:
            ...
            model_period = re.sub(r"\s+", "_", period_source).strip("_") or "UnknownPeriod"
            model_date = ""
    else:
        model_period = re.sub(r"\s+", "_", period_source).strip("_") or "UnknownPeriod"
        model_date = ""
...
def extract_empirical_candidates(
    ...
    penetration_col = anchor_col + offsets["sales_captured_in_db_pct"]
    rel_col = penetration_col - helper_col

    for idx in range(n_quarters):
        row = first_data_row + idx
        formula = f'=IFERROR(AVERAGE(R[{-idx}]C[{rel_col}]:RC[{rel_col}]), "")'
    ...
    for idx in range(n_quarters):
        ...
        if avg_penetration is None:
            q_sales_num = as_number(quarterly_sales)
            reported_num = as_number(reported_sales)
            if q_sales_num is not None and reported_num not in (None, 0):
                avg_penetration = q_sales_num / reported_num
...
def extract_regression_candidates(
    ...
    for idx, n_quarters in enumerate(n_values):
        row = first_data_row + idx
        num_quarters_used = as_int(snapshot.get(row, anchor_col + offsets["num_quarters_used"])) or n_quarters
        forecast_value = safe_value(snapshot.get(row, anchor_col + offsets["forecast_value"]))
