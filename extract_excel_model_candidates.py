...
def build_empirical_rows(
    ...
    avg_pen_by_quarters: dict[int, Any] = {}
    scratch_rows_used: list[int] = []
    ...
    for idx in range(max_iterations):
        ...
        target_row = scratch_row + idx
        scratch_cell = sheet.cells(target_row, scratch_col)
        set_formula2(
            scratch_cell,
            f"=AVERAGE(R{start_row}C{penetration_input_col}:R{end_row}C{penetration_input_col})",
        )
        scratch_rows_used.append(target_row)

    if scratch_rows_used:
        # Batch formula updates to minimize recalc calls.
        workbook.app.calculate()
        for idx, target_row in enumerate(scratch_rows_used):
            avg_pen_by_quarters[idx + 1] = sheet.cells(target_row, scratch_col).value
    ...
    for idx in range(max_iterations):
        ...
        avg_pen_calc = avg_pen_by_quarters.get(n_quarters)

        avg_pen_existing = cell_value(sheet, grid, source_row, avg_pen_col)
        avg_penetration = avg_pen_existing if is_number(avg_pen_existing) else avg_pen_calc
    ...
    try:
        if scratch_rows_used:
            sheet.range(
                (scratch_rows_used[0], scratch_col),
                (scratch_rows_used[-1], scratch_col),
            ).value = None
    except Exception:
        pass
    return rows
...
def build_regression_rows(
    ...
    rows: list[dict[str, Any]] = []
    previous_output_signature: tuple[Any, ...] | None = None
    max_iterations = min(10, len(numeric_pair_rows))
    quarter_windows = list(range(2, max_iterations + 1))
    scratch_rows_used: list[int] = []

    for idx, n_quarters in enumerate(quarter_windows):
        start_row = numeric_pair_rows[-n_quarters]
        end_row = numeric_pair_rows[-1]
        target_row = scratch_row + idx
        intercept_cell = sheet.cells(target_row, intercept_col)
        slope_cell = sheet.cells(target_row, slope_col)
        set_formula2(
            intercept_cell,
    ...
    for idx, n_quarters in enumerate(quarter_windows):
        ...
        is_final_row = idx == len(quarter_windows) - 1
        if is_final_row and previous_output_signature is not None and signature == previous_output_signature:
            continue
        ...
        if all(
            ...
            continue
        rows.append(row)
        previous_output_signature = signature

    try:
        if scratch_rows_used:
            sheet.range(
                (scratch_rows_used[0], intercept_col),
                (scratch_rows_used[-1], slope_col),
            ).value = None
    except Exception:
        pass
    return rows
