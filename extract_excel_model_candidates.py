...
def build_sheet_snapshot(sheet: xw.Sheet) -> SheetSnapshot:
    ...
    for row in values:
        normalized_rows.append(list(row) + [None] * (max_cols - len(row)))
    return SheetSnapshot(
        start_row=used.row,
        start_col=used.column,
...
def set_r1c1_formula2(cell: xw.Range, formula: str) -> None:
    # Prefer Formula2R1C1 to avoid column-letter conversion overhead.
    try:
...
def extract_empirical_candidates(
    ...
    # Offsets are anchored to the 'max' column to avoid repeated sheet scans.
    offsets = {
    ...
    penetration_source_col = anchor_col + offsets["avg_penetration_pct"]
    ...
    for idx in range(n_quarters):
        ...
        set_r1c1_formula2(sheet.range((row, helper_col)), f'=IFERROR(AVERAGE({start_ref}:{end_ref}), "")')
    recalculate(workbook)

    helper_values = [safe_value(sheet.range((first_data_row + idx, helper_col)).value) for idx in range(n_quarters)]
    sheet.range(
        (first_data_row, helper_col),
        (first_data_row + n_quarters - 1, helper_col),
    ).clear_contents()
