...
def to_number(value: Any) -> float | None:
    ...
    try:
        ...
        return None
    return parsed / 100.0 if was_percent else parsed
...
def parse_model_label(file_path: Path) -> ModelLabel:
    stem = file_path.stem
    parts = [piece.strip() for piece in stem.split(" - ") if piece.strip()]

    ticker = "UNKNOWN"
    if len(parts) >= 2:
        parsed_ticker = re.sub(r"[^A-Za-z0-9]", "", parts[1]).upper()
        if parsed_ticker:
    ...
    if period_match:
        ...
        model_period = f"{cadence.capitalize()}{month_text.capitalize()}_{year}"
        model_date = date(year, MONTH_NUM[month_text], MODEL_DAY[cadence]).isoformat()
    else:
...
def list_input_files(source_dir: Path) -> tuple[list[Path], int]:
    ...
    for file_path in sorted(source_dir.iterdir()):
        ...
        if file_path.suffix.lower() != ".xlsx":
            print(f"Skipping {file_path.name}: not .xlsx")
            skipped += 1
            continue
...
def build_empirical_rows(
    ...
    # Write helper formulas once, calculate once, then read values.
    ...
    for n_quarters in range(1, 11):
        ...
        if is_effectively_empty(blank_probe):
            continue
...
def build_regression_rows(
    ...
    blank_streak = 0
    for row in range(anchor_row + 1, anchor_row + 51):
        num_quarters_used = ws.range((row, cols["num_quarters_used"])).value
        forecast_value = ws.range((row, cols["forecast_value"])).value
        forecast_max = ws.range((row, cols["forecast_max"])).value
        forecast_min = ws.range((row, cols["forecast_min"])).value
...
def write_output_workbook(
    ...
    for row in empirical_rows:
        ws_emp.append([row.get(column) for column in EMPIRICAL_HEADERS])

    ws_reg.append(REGRESSION_HEADERS)
    for row in regression_rows:
        ws_reg.append([row.get(column) for column in REGRESSION_HEADERS])
...
def run() -> None:
    ...
    try:
        for file_path in files:
            ...
            try:
                empirical, regression = process_file(app=app, file_path=file_path)
