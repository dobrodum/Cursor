...
def process_empirical_sheet(
    ...
    for row, fallback_num_quarters in rows_to_read:
        num_quarters_used = (
            to_int(matrix_cell(matrix, start_row, start_col, row, num_quarters_col))
            or fallback_num_quarters
        )
        if num_quarters_used < 1:
            num_quarters_used = fallback_num_quarters
        ...
        forecast_max = matrix_cell(matrix, start_row, start_col, row, forecast_max_col)
        forecast_min = matrix_cell(matrix, start_row, start_col, row, forecast_min_col)
        quarterly_sales = matrix_cell(matrix, start_row, start_col, row, quarterly_sales_col)
        reported_sales = matrix_cell(matrix, start_row, start_col, row, reported_sales_col)
        growth_rate_pct = matrix_cell(matrix, start_row, start_col, row, growth_rate_col)
        sales_captured_in_db_pct = matrix_cell(
            matrix, start_row, start_col, row, sales_captured_col
        )
        last_quarter_used = matrix_cell(matrix, start_row, start_col, row, last_quarter_col)
...
def process_regression_sheet(
    ...
    for idx, (row, fallback_num_quarters) in enumerate(rows_to_read):
        ...
        forecast_value = matrix_cell(matrix, start_row, start_col, row, forecast_total_col)
        forecast_max = matrix_cell(matrix, start_row, start_col, row, forecast_max_col)
        forecast_min = matrix_cell(matrix, start_row, start_col, row, forecast_min_col)
        actual_value = matrix_cell(matrix, start_row, start_col, row, actual_value_col)
