...
def parse_file_label(file_path: Path) -> FileLabel:
    ...
    if not period_match:
        raise ValueError("filename missing Early/Mid/Late + month + year token")
...
def build_empirical_rows(
    ...
    for n_quarters in range(1, max_iterations + 1):
        ...
        target_row = scratch_row + n_quarters - 1
        sheet.cells(target_row, scratch_col).formula2 = (
            f"=AVERAGE(R{start_row}C{penetration_input_col}:R{end_row}C{penetration_input_col})"
        )
        formula_rows[n_quarters] = target_row

    if formula_rows:
        workbook.app.calculate()

    avg_pen_calc_by_n: dict[int, Any] = {}
    for n_quarters, target_row in formula_rows.items():
        avg_pen_calc_by_n[n_quarters] = sheet.cells(target_row, scratch_col).value
    ...
    for idx in range(max_iterations):
        ...
        avg_penetration = avg_pen_existing if is_number(avg_pen_existing) else avg_pen_calc_by_n.get(
            n_quarters
        )
    ...
    if formula_rows:
        first_clear_row = min(formula_rows.values())
        last_clear_row = max(formula_rows.values())
        try:
            sheet.range((first_clear_row, scratch_col), (last_clear_row, scratch_col)).value = None
        except Exception:
            pass
    return rows
...
def build_regression_rows(
    ...
    # Required offsets from the max anchor.
    y_col = anchor_col - 7
    x_col = anchor_col - 11
    ...
    regression_params_by_n: dict[int, tuple[Any, Any]] = {}
    for n_quarters, target_row in formula_rows.items():
        intercept_value = sheet.cells(target_row, intercept_col).value
        slope_value = sheet.cells(target_row, slope_col).value
        regression_params_by_n[n_quarters] = (intercept_value, slope_value)
    ...
    for n_quarters in range(2, max_iterations + 1):
        ...
        num_quarters_used = int(num_quarters_raw) if is_number(num_quarters_raw) else n_quarters
        forecast_value = cell_value(sheet, grid, source_row, forecast_col)
        actual_value = cell_value(sheet, grid, source_row, actual_col)
        forecast_max = cell_value(sheet, grid, source_row, max_col)
        forecast_min = cell_value(sheet, grid, source_row, min_col)
        intercept_value, slope_value = regression_params_by_n.get(n_quarters, (None, None))
    ...
    if formula_rows:
        ...
        last_clear_row = max(formula_rows.values())
        try:
            sheet.range((first_clear_row, intercept_col), (last_clear_row, slope_col)).value = None
        except Exception:
            pass
    return rows
