...
Workflow:
1) Use one hidden Excel app for the full run.
2) Open each source workbook once, process both target sheets while open, close without saving.
3) Write one output workbook with:
...
def set_formula2_r1c1(cell: Any, formula: str) -> None:
    """Write an R1C1 formula using formula2 first, with safe fallbacks."""
    try:
...
def parse_file_label(file_name: str) -> dict[str, str]:
    """Parse ticker, model_period, model_date, and model from file name."""
...
def process_regression_sheet(
    ...
    x_col = anchor_col - 11
    num_quarters_col = anchor_col - 11
    forecast_total_col = anchor_col - 3  # TOT FCST w/o SA in model layout.
