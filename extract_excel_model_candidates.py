...
def parse_file_labels(file_path: Path) -> FileLabels:
    ...
    period_source = parts[-1] if parts else stem
    period_source = re.sub(r"(?i)[_\-\s]*send$", "", period_source).strip()
    period_source = period_source or "UnknownPeriod"

    period_match = re.search(r"(Early|Mid|Late)\s*([A-Za-z]{3,})\s*(\d{4})", period_source, re.IGNORECASE)
    if not period_match:
    ...
    period_part = period_match.group(1).lower()
...
def extract_empirical_candidates(
    ...
    for idx in range(n_quarters):
        ...
        avg_penetration = avg_penetration_existing if avg_penetration_existing is not None else helper_values[idx]
