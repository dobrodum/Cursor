...
def calc_bounds(
    ...
    # Nearby max/min can be multipliers (for example 1.1/0.9) or absolute values.
...
def build_output_path(src_input_dir: Path, src_output_dir: Path) -> Path:
    ...
    out_path = src_output_dir / f"{base_name}.xlsx"
    if not out_path.exists():
        return out_path

    idx = 1
    while True:
        out_path = src_output_dir / f"{base_name}.{idx}.xlsx"
        if not out_path.exists():
            return out_path
        idx += 1
...
def parse_file_label(file_path: Path) -> Dict[str, str]:
    ...
    period_token = re.sub(r"[_\-\s]*send$", "", period_token.strip(), flags=re.IGNORECASE)
    ...
    ticker = ticker.strip().upper() if ticker else "UNKNOWN"
...
def close_workbook_without_save(wb: xw.Book) -> None:
    ...
    # Some versions accept a positional boolean.
    try:
        wb.close(False)
        return
    except Exception:
...
def extract_empirical_rows(
    ...
    for num_quarters in range(1, N_QUARTERS + 1):
        ...
        growth_rate_pct = safe_div(
            None
            if reported_sales is None or previous_reported_sales is None
            else reported_sales - previous_reported_sales,
            previous_reported_sales,
        )
        sales_captured_in_db_pct = safe_div(quarterly_sales, reported_sales)
        forecast_value = safe_div(quarterly_sales, avg_penetration_pct)  # estimated total sold
...
def extract_regression_rows(
    ...
    for num_quarters in range(1, N_QUARTERS + 1):
        ...
        forecast_max, forecast_min = calc_bounds(forecast_total_without_sa, raw_max, raw_min)
...
def format_output_sheet(writer: pd.ExcelWriter, sheet_name: str, df: pd.DataFrame) -> None:
    ...
    for col_idx, col_name in enumerate(df.columns, start=1):
        samples = [str(v) for v in df[col_name].dropna().head(300)]
        max_len = max([len(col_name)] + [len(v) for v in samples]) if samples else len(col_name)
...
def main() -> None:
    ...
    # Manual calculation mode can reduce calc overhead on large batches.
