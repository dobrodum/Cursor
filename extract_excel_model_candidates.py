...
def extract_empirical_candidates(
    ...
    # Anchor-based offsets from the "max" column in the model layout.
    offsets = {
    ...
    # Formula scratch area (temporary; cleared before close).
...
def extract_regression_candidates(
    ...
    for idx, n in enumerate(n_values):
        ...
        is_last_row = idx == len(n_values) - 1
        if is_last_row and previous_signature is not None and signature == previous_signature:
            continue
...
def run() -> None:
    ...
    output_artifact_pattern = re.compile(
    ...
    try:
        ...
        for file_path in sorted(input_path.iterdir()):
            ...
            if file_path.suffix.lower() != ".xlsx":
                ...
                continue
            if output_artifact_pattern.match(file_path.name):
                print(f"Skipped file: {file_path.name} (output artifact)")
                continue
