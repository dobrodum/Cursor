...
def get_empirical_rows(sheet: xw.Sheet, wb: xw.Book, meta: FileMeta, source_file: str) -> list[dict[str, Any]]:
    ...
    max_col = resolve_col(header_map, ["max", "forecast_max"], anchor_col, EMPIRICAL_FALLBACK_OFFSETS, "forecast_max")
    min_col = resolve_col(header_map, ["min", "forecast_min"], anchor_col, EMPIRICAL_FALLBACK_OFFSETS, "forecast_min")
...
def get_regression_rows(sheet: xw.Sheet, wb: xw.Book, meta: FileMeta, source_file: str) -> list[dict[str, Any]]:
    ...
    # Required by the extraction rules.
    y_col = anchor_col - 7
    x_col = anchor_col - 11
