...
def next_output_path(input_path: Path, output_path: Path) -> Path:
    ...
    idx = 1
    while True:
        next_candidate = output_path / f"{input_path.name}_PARAM.{idx}.xlsx"
...
def find_offset(
    ...
    fragment_list = [normalize_text(item) for item in fragments]
    best_score = float("inf")
    best_offset: Optional[int] = None
    for text_label, positions in snapshot.text_positions.items():
        if not any(fragment in text_label for fragment in fragment_list):
            continue
        for row, col in positions:
            score = (abs(row - anchor_row) * 100) + abs(col - anchor_col)
            if score < best_score:
                best_score = score
                best_offset = col - anchor_col
    return best_offset if best_offset is not None else default
...
def read_block(
    ...
    if bottom_row < top_row or right_col < left_col:
        return []
    raw = sheet.range((top_row, left_col), (bottom_row, right_col)).value
    block = ensure_2d(raw)
    expected_rows = bottom_row - top_row + 1
    expected_cols = right_col - left_col + 1
    if len(block) < expected_rows:
        block.extend([[None] * expected_cols for _ in range(expected_rows - len(block))])
    for idx, row in enumerate(block):
        if len(row) < expected_cols:
            block[idx] = row + [None] * (expected_cols - len(row))
    return block
...
def prepare_empirical_formulas(
    ...
    helper_values = sheet.range(
        (helper_start_row, helper_col),
        (helper_start_row + N_QUARTERS - 1, helper_col),
    ).value
    helper_matrix = ensure_2d(helper_values)
    output: List[Optional[float]] = []
    for row in helper_matrix:
        output.append(to_float(row[0] if row else None))
    while len(output) < N_QUARTERS:
        output.append(None)
    return output
...
def extract_empirical_rows(
    ...
    for idx in range(N_QUARTERS):
        ...
        avg_penetration = avg_formula_values[idx]
        if avg_penetration is None:
            avg_penetration = to_float(
                value_from_block(
                    block,
                    row=row_num,
...
def prepare_regression_formulas(
    ...
    intercept_list = [to_float(row[0] if row else None) for row in intercept_values]
    slope_list = [to_float(row[0] if row else None) for row in slope_values]
    while len(intercept_list) < N_QUARTERS:
        intercept_list.append(None)
    while len(slope_list) < N_QUARTERS:
        slope_list.append(None)
    return intercept_list, slope_list
