...
def get_block_values(
    ...
    if end_row < start_row or end_col < start_col:
        return []
    return to_2d(sheet.range((start_row, start_col), (end_row, end_col)).value)


def value_from_block(row_values: List[Any], block_start_col: int, absolute_col: int) -> Any:
    index = absolute_col - block_start_col
...
def process_empirical_sheet(
    ...
    block_start_col = min(value_cols)
    block_end_col = max(value_cols)
    data_block = get_block_values(sheet, row_start, row_end, block_start_col, block_end_col)
    avg_penetration_values = get_column_values(sheet, temp_avg_col, row_start, row_end)
    default_last_quarter_used = sheet.range((penetration_rows[-1], quarter_label_col)).value if penetration_rows else None

    for index in range(max_n):
        n_quarters = index + 1
        row_values = data_block[index] if index < len(data_block) else []

        avg_penetration_pct = avg_penetration_values[index] if index < len(avg_penetration_values) else None
        num_quarters_used = value_from_block(row_values, block_start_col, num_quarters_col)
        if not is_number(num_quarters_used):
            num_quarters_used = n_quarters

        forecast_value = value_from_block(row_values, block_start_col, forecast_value_col)
        reported_sales = value_from_block(row_values, block_start_col, actual_value_col)
        forecast_max = value_from_block(row_values, block_start_col, forecast_max_col)
        forecast_min = value_from_block(row_values, block_start_col, forecast_min_col)
        quarterly_sales = value_from_block(row_values, block_start_col, quarterly_sales_col)
        growth_rate_pct = value_from_block(row_values, block_start_col, growth_rate_col)
        sales_captured_in_db_pct = value_from_block(row_values, block_start_col, sales_captured_col)
        last_quarter_used = value_from_block(row_values, block_start_col, quarter_label_col)
        if last_quarter_used in (None, ""):
            last_quarter_used = default_last_quarter_used
...
def process_regression_sheet(
    ...
    block_end_col = max(value_cols)
    data_block = get_block_values(sheet, row_start, row_end, block_start_col, block_end_col)
    intercept_values = get_column_values(sheet, temp_intercept_col, row_start, row_end)
    slope_values = get_column_values(sheet, temp_slope_col, row_start, row_end)

    for index in range(max_n):
        n_quarters = index + 1
        row_values = data_block[index] if index < len(data_block) else []

        intercept = intercept_values[index] if index < len(intercept_values) else None
        slope = slope_values[index] if index < len(slope_values) else None
        ...
        forecast_max = value_from_block(row_values, block_start_col, forecast_max_col)
        forecast_min = value_from_block(row_values, block_start_col, forecast_min_col)
        actual_value = value_from_block(row_values, block_start_col, actual_value_col)
        num_quarters_used = value_from_block(row_values, block_start_col, num_quarters_col)
        if not is_number(num_quarters_used):
            num_quarters_used = n_quarters
