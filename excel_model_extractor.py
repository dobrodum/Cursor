#!/usr/bin/env python3
"""
Batch-extract empirical and regression model candidates from .xlsx workbooks.

Behavior summary:
- Uses one hidden Excel app for the full run.
- Opens each source workbook once, extracts both model sheets, then closes safely.
- Never saves or modifies source workbooks.
- Builds one output workbook with:
...
def next_output_path(input_folder: Path, out_folder: Path) -> Path:
    ...
    first = out_folder / f"{base_name}.xlsx"
    if not first.exists():
        return first
...
def find_anchor_cell(sheet: xw.Sheet, anchor_text: str = "max") -> Optional[Tuple[int, int, int, int, int, int]]:
    """
    Returns:
        (anchor_row, anchor_col, used_first_row, used_first_col, used_last_row, used_last_col)
    """
...
def build_header_map(sheet: xw.Sheet, header_row: int, start_col: int, end_col: int) -> Dict[str, int]:
    ...
    for idx, raw in enumerate(values):
        ...
        if key:
            mapped[key] = start_col + idx
    return mapped


def resolve_column(header_map: Dict[str, int], aliases: Sequence[str]) -> Optional[int]:
    norm_aliases = [normalize_key(alias) for alias in aliases]
...
def extract_empirical_rows(workbook: xw.Book, labels: FileLabels, source_file: str) -> List[Dict[str, Any]]:
    ...
    anchor_row, anchor_col, _, used_first_col, _, used_last_col = anchor
    ...
    # Existing empirical logic uses max anchor plus offsets.
    columns: Dict[str, Optional[int]] = {
        "forecast_max": anchor_col,
        "forecast_min": resolve_column(header_map, ["min", "forecast min", "minimum"]),
        "num_quarters_used": resolve_column(
            header_map, ["num quarters used", "quarters used", "num quarters", "n quarters"]
        ),
        "last_quarter_used": resolve_column(
            header_map, ["last quarter used", "last quarter", "last qtr", "last quarter in sample"]
        ),
        "forecast_value": resolve_column(
            header_map, ["estimated total sold", "forecast value", "forecast", "total forecast", "tot fcst"]
        ),
        "actual_value": resolve_column(header_map, ["reported sales", "actual value", "actual", "reported"]),
        "avg_penetration_pct": resolve_column(
            header_map, ["avg penetration pct", "average penetration", "avg penetration", "penetration"]
        ),
        "quarterly_sales": resolve_column(header_map, ["quarterly sales", "quarter sales", "sales"]),
        "reported_sales": resolve_column(header_map, ["reported sales", "reported"]),
        "growth_rate_pct": resolve_column(header_map, ["growth rate pct", "growth rate", "growth"]),
        "sales_captured_in_db_pct": resolve_column(
            header_map, ["sales captured in db pct", "sales captured in db", "captured in db"]
        ),
    }
    ...
    for idx in range(1, N_QUARTERS + 1):
        row = anchor_row + idx
        if (
            columns["quarterly_sales"] is None
            and columns["reported_sales"] is None
            and columns["avg_penetration_pct"] is None
        ):
            break
    ...
    for idx in range(1, N_QUARTERS + 1):
        ...
        num_quarters_used = sheet.cells(row, columns["num_quarters_used"]).value if columns["num_quarters_used"] else idx
        ...
        last_quarter_used = sheet.cells(row, columns["last_quarter_used"]).value if columns["last_quarter_used"] else None
        quarterly_sales = sheet.cells(row, columns["quarterly_sales"]).value if columns["quarterly_sales"] else None
        reported_sales = sheet.cells(row, columns["reported_sales"]).value if columns["reported_sales"] else None
        growth_rate_pct = sheet.cells(row, columns["growth_rate_pct"]).value if columns["growth_rate_pct"] else None
        sales_captured_in_db_pct = (
            sheet.cells(row, columns["sales_captured_in_db_pct"]).value if columns["sales_captured_in_db_pct"] else None
        )
        avg_penetration_pct = sheet.cells(row, temp_avg_col).value if row in formula_rows else None
...
def find_last_history_row(sheet: xw.Sheet, anchor_row: int, x_col: int, y_col: int) -> Optional[int]:
    for row in range(anchor_row - 1, 0, -1):
        x_val = sheet.cells(row, x_col).value
        y_val = sheet.cells(row, y_col).value
...
def extract_regression_rows(workbook: xw.Book, labels: FileLabels, source_file: str) -> List[Dict[str, Any]]:
    ...
    columns: Dict[str, Optional[int]] = {
        ...
        "forecast_value": resolve_column(
            header_map, ["tot fcst w/o sa", "tot fcst wo sa", "forecast total without sa", "forecast without sa"]
        ),
        "actual_value": resolve_column(header_map, ["actual", "reported sales", "actual value"]),
    }
    ...
    for idx in range(1, N_QUARTERS + 1):
        row = anchor_row + idx
        num_quarters = sheet.cells(row, columns["num_quarters_used"]).value if columns["num_quarters_used"] else idx
        num_quarters_int = to_int(num_quarters) or idx
        if last_history_row is None or num_quarters_int <= 0:
            continue
    ...
    for idx in range(1, N_QUARTERS + 1):
        ...
        forecast_max = sheet.cells(row, columns["forecast_max"]).value if columns["forecast_max"] else None
        forecast_min = sheet.cells(row, columns["forecast_min"]).value if columns["forecast_min"] else None
        intercept = sheet.cells(row, temp_intercept_col).value if row in formula_rows else None
        slope = sheet.cells(row, temp_slope_col).value if row in formula_rows else None
...
def write_sheet(worksheet: Worksheet, headers: Sequence[str], rows: Sequence[Dict[str, Any]]) -> None:
...
def main() -> None:
    if xw is None or Workbook is None or Font is None or get_column_letter is None:
        raise ModuleNotFoundError(
            "xlwings and openpyxl are required. Install with: pip install xlwings openpyxl"
