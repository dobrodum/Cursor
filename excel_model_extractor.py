...
This script:
1. Opens each source workbook only once using one hidden Excel app.
2. Processes "Empirical Model" and "Regression Model" while open.
3. Writes one output workbook with:
...
def parse_filename_label(file_path: Path) -> Dict[str, str]:
    ...
    match = re.search(
        ...
        r"(?P<period>(Early|Mid|Late)([A-Za-z]{3})(\d{4}))",
        stem,
        flags=re.IGNORECASE,
    )
    ...
    if not match:
        ...
        split_parts = [part.strip() for part in stem.split("-") if part.strip()]
...
def iter_source_files(src_input_dir: Path) -> Iterable[Path]:
    ...
    for file_path in sorted(src_input_dir.iterdir()):
        ...
        if output_pattern.match(file_path.name):
            print(f"Skipped file: {file_path.name} (output workbook pattern)")
            continue
        yield file_path
    ...
    for row_idx, row_values in enumerate(values_2d):
        for col_idx, value in enumerate(row_values):
            if normalize_text(value) == anchor_norm:
                return sheet.range((start_row + row_idx, start_col + col_idx))
    return None
...
def collect_header_cells(
    ...
    block = to_2d(sheet.range((start_row, start_col), (end_row, end_col)).value)
    cells: List[Tuple[int, int, str]] = []
    for row_offset, row_values in enumerate(block):
        for col_offset, value in enumerate(row_values):
            norm = normalize_text(value)
            if norm:
                cells.append((start_row + row_offset, start_col + col_offset, norm))
    return cells
...
def set_formula2_r1c1(cell: xw.Range, formula_r1c1: str) -> None:
    ...
    # Safe fallbacks for environments where formula2 behavior varies.
...
def extract_empirical_rows(
    sheet: xw.Sheet, meta: Dict[str, str], source_file: str, n_quarters: int = N_QUARTERS
) -> List[Dict[str, Any]]:
    ...
    for key, key_aliases in aliases.items():
        found_col = find_column_by_aliases(header_cells, key_aliases)
        offsets[key] = (found_col - anchor_col) if found_col is not None else defaults.get(key)

    data_start_row = anchor_row + 1
    data_rows = [data_start_row + idx for idx in range(n_quarters)]
    ...
    # Build all formula writes first, then calculate once for speed.
    for row in data_rows:
        if penetration_source_col is not None:
            ...
            numerator_off = quarterly_sales_col - temp_col
            denominator_off = reported_sales_col - temp_col
            avg_formula = f'=IFERROR(RC[{numerator_off}]/RC[{denominator_off}],"")'
        else:
            avg_formula = '=""'
        set_formula2_r1c1(sheet.range((row, temp_col)), avg_formula)

    sheet.book.app.calculate()

    rows: List[Dict[str, Any]] = []
    for idx, row in enumerate(data_rows, start=1):
        num_quarters_used = value_at(
            sheet, row, offset_to_col(anchor_col, offsets.get("num_quarters_used"))
        )
        if is_blank(num_quarters_used):
            num_quarters_used = idx

        last_quarter_used = value_at(
            sheet, row, offset_to_col(anchor_col, offsets.get("last_quarter_used"))
        )
        ...
        if is_blank(avg_penetration_pct):
            avg_penetration_pct = value_at(
                sheet, row, offset_to_col(anchor_col, offsets.get("avg_penetration_pct"))
            )
        ...
        sales_captured_in_db_pct = value_at(
            sheet, row, offset_to_col(anchor_col, offsets.get("sales_captured_in_db_pct"))
        )
...
def extract_regression_rows(
    ...
    # Build formulas once, then calculate once.
    for row in data_rows:
        intercept_formula = (
    ...
    for idx, row in enumerate(data_rows, start=1):
        ...
        forecast_max = value_at(sheet, row, offset_to_col(anchor_col, offsets.get("forecast_max")))
        forecast_min = value_at(sheet, row, offset_to_col(anchor_col, offsets.get("forecast_min")))
        intercept = value_at(sheet, row, intercept_col)
        slope = value_at(sheet, row, slope_col)

        if all(is_blank(v) for v in (forecast_value, forecast_max, forecast_min, intercept, slope)):
            continue

        dedupe_key = normalized_key_for_dedupe(
...
def write_sheet(
    workbook: Workbook, title: str, columns: Sequence[str], rows: Sequence[Dict[str, Any]]
) -> None:
