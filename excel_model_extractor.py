...
try:
    ...
    raise SystemExit(
        "Missing dependency: xlwings is required to run this script."
    ) from exc
...
# Anchor-based offsets used first, with label-based fallback for robustness.
...
def extract_numeric_series(label_cell: Any) -> NumericSeries | None:
    ...
    if not options:
        return None
    return max(options, key=lambda s: len(s.cells))
...
def extract_empirical_rows(
    ...
    for num_quarters in range(1, num_candidates + 1):
        ...
        actual_value = number_or_blank(
            context.reported_sales_cell.value if context.reported_sales_cell else None
        )
        forecast_max = number_or_blank(
            context.forecast_max_cell.value if context.forecast_max_cell else None
        )
        forecast_min = number_or_blank(
            context.forecast_min_cell.value if context.forecast_min_cell else None
        )
        quarterly_sales = number_or_blank(
            context.quarterly_sales_cell.value if context.quarterly_sales_cell else None
        )
        ...
        sales_captured = number_or_blank(
            context.sales_captured_cell.value if context.sales_captured_cell else None
        )
...
def extract_regression_rows(
    ...
    for num_quarters in range(1, max_candidates + 1):
        ...
        forecast_value = number_or_blank(
            context.forecast_wo_sa_cell.value if context.forecast_wo_sa_cell else None
        )
