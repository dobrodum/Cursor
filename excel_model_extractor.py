#!/usr/bin/env python3
"""
Batch extract model candidates from all .xlsx files in an input directory.

Design goals:
- Open each source workbook once.
- Process "Empirical Model" and "Regression Model" while still open.
- Close source workbooks without saving.
- Write one output workbook containing:
...
def parse_file_labels(file_path: Path) -> FileLabels:
    ...
    pattern = r"(?i)\b(early|mid|late)(jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)(\d{4})\b"
    period_match = re.search(pattern, period_source)
    if period_match is None:
        period_match = re.search(pattern, stem)
...
def extract_regression_rows(workbook: xw.Book, labels: FileLabels, source_file: str) -> List[Dict[str, Any]]:
    ...
    header_start_col = max(used_first_col, anchor_col - 20)
    header_end_col = max(used_last_col, anchor_col + 20)
    header_map = build_header_map(sheet, anchor_row, header_start_col, header_end_col)
...
def main() -> None:
    if xw is None or Workbook is None or Font is None or get_column_letter is None:
        raise ModuleNotFoundError(
            "xlwings and openpyxl are required. Install them with: pip install xlwings openpyxl"
