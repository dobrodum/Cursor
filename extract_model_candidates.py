...
- each source workbook is opened only once
...
def parse_filename_metadata(file_path: Path) -> Dict[str, str]:
    ...
    model_date = ""
    if token_match:
        part = token_match.group(1).title()
...
def extract_empirical_rows(
    ...
    if not matrix:
        print(f"Skipped Empirical Model in {source_file}: sheet empty")
        return rows
    ...
    # Required: loop n_quarters=10 and use R1C1 + formula2.
    for n_quarters in range(1, N_QUARTERS + 1):
        ...
        sheet.range((out_row, helper_col)).formula2 = avg_formula
        sheet.range((out_row, helper_forecast_col)).formula2 = forecast_formula
...
def extract_regression_rows(
    ...
    matrix = normalize_2d(used.value)
    if not matrix:
        print(f"Skipped Regression Model in {source_file}: sheet empty")
        return rows
    ...
    helper_col = max(base_col + max_width + 2, anchor_col + 2)
    helper_slope_col = helper_col + 1
    formulas_written = False

    # Required: loop n_quarters=10 and use R1C1 + formula2 for INTERCEPT/SLOPE.
    for n_quarters in range(1, N_QUARTERS + 1):
        out_row = anchor_row + n_quarters
        hist_end = history_rows[-1]
        hist_start = history_rows[max(0, len(history_rows) - n_quarters)]
        ...
        sheet.range((out_row, helper_col)).formula2 = intercept_formula
        sheet.range((out_row, helper_slope_col)).formula2 = slope_formula
        formulas_written = True
    ...
    for n_quarters in range(1, N_QUARTERS + 1):
        ...
        intercept = safe_cell_read(sheet, out_row, helper_col)
        ...
        # Required: prevent duplicate final row by comparing to previous row.
