...
def parse_model_metadata(file_path: Path) -> dict[str, str]:
    ...
    if period_match:
        ...
        model_date = date(year, month, DAY_BY_PERIOD[period_tag.lower()]).isoformat()
    else:
        compact = re.sub(r"\s+", "", period_token)
        model_period = compact.replace("-", "_")
        model_date = ""
...
def collect_history_points(
    ...
    start = max(row_start, snapshot.start_row)
    end = min(row_end, snapshot.end_row)
...
def extract_empirical_candidates(
    ...
    captured_col = find_column(headers, ["captured", "db"]) or find_column(headers, ["captured"]) or (
        anchor_col + 3
    )
    ...
    n_quarters = 10
    max_quarters = min(n_quarters, len(history))
    ...
    for window_size in range(1, max_quarters + 1):
        subset = history[-window_size:]
        ...
        table_row = anchor_row + window_size
        ...
        num_quarters_used = table_num_quarters if table_num_quarters is not None else window_size
...
def extract_regression_candidates(
    ...
    for window_size in range(2, max_quarters + 1):
        subset = history[-window_size:]
        first_row = subset[0].row
        last_row = subset[-1].row
...
def write_sheet(worksheet: Any, columns: list[str], rows: list[dict[str, Any]]) -> None:
    ...
    worksheet.auto_filter.ref = f"A1:{last_col_letter}{max(1, worksheet.max_row)}"
    ...
    for idx, column in enumerate(columns, start=1):
        ...
        for row in rows:
            ...
            if text_len > max_len:
                max_len = text_len
        worksheet.column_dimensions[get_column_letter(idx)].width = min(max(max_len + 2, 12), 48)
...
def main() -> None:
    ...
    try:
        for file_path in source_files:
            ...
            try:
                # Source workbook is opened exactly once and reused for both model sheets.
