...
2) Processes both "Empirical Model" and "Regression Model" while workbook is open.
...
def empirical_rows_from_sheet(
    ...
    for n_quarters in range(1, N_QUARTERS + 1):
        ...
        sales_captured_in_db_pct = to_float(read_cell_value(sheet, row, cols["sales_captured_in_db_pct"]))
...
def regression_rows_from_sheet(
    ...
    # Prevent duplicate final row by comparing values to previous row.
    if len(rows) >= 2:
        prev = rows[-2]
        curr = rows[-1]
                ...
                if "Empirical Model" in sheet_names:
                    empirical_rows.extend(
                        empirical_rows_from_sheet(
                            workbook,
                            workbook.sheets["Empirical Model"],
                            labels,
                            file_path.name,
                        )
                    )
                ...
                if "Regression Model" in sheet_names:
                    regression_rows.extend(
                        regression_rows_from_sheet(
                            workbook,
                            workbook.sheets["Regression Model"],
                            labels,
                            file_path.name,
                        )
                    )
