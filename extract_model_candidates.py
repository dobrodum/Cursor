...
def parse_file_labels(file_name: str) -> Dict[str, str]:
    """
    Expected source filename pattern example:
...
def process_empirical_sheet(
    ...
    if captured_col is not None:
        for row in range(data_start, data_end + 1):
            ...
            formula = f"=AVERAGE(R{start_formula_row}C{captured_col}:R{row}C{captured_col})"
    ...
    for row in range(data_start, data_end + 1):
        ...
        last_quarter_used = sheet.range((row, last_q_col)).value if last_q_col is not None else None
